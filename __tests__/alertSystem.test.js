import AlertSystem from '../backend/controllers/alertSystem/alertSystem';

// Mock the AlertSystem
jest.mock('../backend/controllers/alertSystem/alertSystem');
    ...
    try {
      ...
      const metrics = [
        {
          metricName: 'diskSpace',
          metricQuery:
            'round(sum(container_fs_usage_bytes{job="localprometheus"}) by (container_name) / sum(container_fs_limit_bytes{job="localprometheus"}) by (container_name), 0.001)',
        },
        {
          metricName: 'memory',
          metricQuery:
            'round((1 - (node_memory_MemAvailable_bytes{job="localprometheus"} / node_memory_MemTotal_bytes{job="localprometheus"})), 0.001)',
        },
        {
          metricName: 'CPU_usage',
          metricQuery:
            'sum(rate(process_cpu_seconds_total{job="localprometheus"}[5m])) * 100',
        },
      ];
      ...
      for (const metric of metrics) {
        ...
        if (data.data && data.data.result && data.data.result[0]) {
          metricsData[metric.metricName] = parseFloat(
            data.data.result[0].value[1]
          );
          console.log(
            `${metric.metricName} value:`,
            metricsData[metric.metricName]
          );
        } else {
  ...
  describe('startMonitoring', () => {
    ...
    it('should not start monitoring if already running', () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        'Metrics monitoring is already running'
      );
    });
  });
